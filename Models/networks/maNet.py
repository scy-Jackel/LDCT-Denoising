...
class WGAN_MA_AE(nn.Module):
    def __init__(self, args, in_ch=1, out_ch=1, feature_scale=4, is_deconv=True, is_batchnorm=True,
        ...
        super(WGAN_MA_AE, self).__init__()
        self.root_path = r'.'
        # self.generator = normalCNN(in_ch, out_ch).cuda()
        maskAttentionBlock = Comprehensive_Atten_Unet(args, in_ch=in_ch, out_ch=out_ch, feature_scale=feature_scale,
                                                  is_deconv=is_deconv, is_batchnorm=is_batchnorm,
                                                  nonlocal_mode=nonlocal_mode, attention_dsample=attention_dsample).cuda()
        self.generator = normalCNN(in_ch*2, out_ch).cuda()
        ...
        attentionBlock_path = self.root_path + '/saved_models/aapm_binary/l1loss/100_aapm_binary_checkpoint.pth.tar'
        self.attentionBlock = update_attentionBlock(maskAttentionBlock, attentionBlock_path)
        # self.lambda_gp = 240+160
        self.indices=torch.tensor([0]).cuda()
    ...
    def d_loss(self, x, y, gp=True, return_gp=False):
        ...
        discriminator loss
        """
        M = self.attentionBlock(x)
        M = torch.index_select(M, 1, self.indices)
        fake = self.generator(x, M)
        fake = fake.detach()
...
def update_attentionBlock(model, model_reload_path):
    if os.path.isfile(model_reload_path):
        print("Loading ATTENTION BLOCK...")
...
def update_model(model, optimizer_g, optimizer_d, model_path, epoch):
    time.sleep(3)
    model_reload_path = os.path.join(model_path, 'model', 'lsgan_{}.pkl'.format(epoch))
