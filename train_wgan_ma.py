...
def train(model, epoch, optimizer_g, optimizer_d, dataloaders, args):
    ...
    d_loss_all = 0
    gp_loss_all = 0
    for p in model.attentionBlock.parameters():
...
def train_model(model,dataloaders,args):
    ...
    scheduler_g = StepLR(optimizer_g, step_size=50, gamma=0.3)
    scheduler_d = StepLR(optimizer_d, step_size=50, gamma=0.3)
    ...
    for epoch in range(args.start_epoch, args.epoch_num):
        ...
        if math.fmod(epoch, 5) == 0:
            torch.save(model.state_dict(), args.ckpt + "/model/" + "wgan_ma_ae_{}.pkl".format(epoch))
...
def main(args):
    ...
    data_length = {'train':2000, 'val':2000, 'test':40}
    patch_size = 256
    args.num_input = 1
    args.num_classes = 1
...
if __name__ == '__main__':
    ...
    args.ckpt = os.path.join(args.ckpt, 'wgan_ma_final')
    print('Models are saved at %s' % (args.ckpt))
    args.save_img_path = os.path.join(args.save_img_path, 'wgan_ma_final')
