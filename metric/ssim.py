...
from metric.metric_util import fspecial_gauss, prepare_image
...
def compute_ssim(quarter, pred, full):
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')

    quater = prepare_image(quarter).to(device)
    pred = prepare_image(pred).to(device)
    full = prepare_image(full).to(device)

    model = SSIM(channels=3)

    quarter_ssim = model(quater, full, as_loss=False).item()
    full_ssim = model(pred, full, as_loss=False).item()
    return quarter_ssim, full_ssim
